...
//setear currentPage a 1 al hacer algun filtro o busqueda para que seleccione la pagina uno siempre
  ...
  return (
    <div>
      ...
      <div style={{display:"flex", gap:"20px", flexWrap:"wrap", margin: "20px 0px"}}>
      
      {/*Boton Prev*/}
      <button onClick={() => {numPag(currentPage -1); setCurrentPage(currentPage -1)}} disabled={currentPage===1} style={currentPage===1 ? {backgroundColor:"#ffffff", color:"gray", cursor:"not-allowed"}: {color:"black"}}> Prev </button>


      {/* aqui hago map del arreglo para que me muestre los botones de la paginacion */}
      {arreglo.map(num => {return <button key={num+"d"} onClick={() => {numPag(num); setCurrentPage(num)}} style={currentPage===num ? {backgroundColor:'green', width:"30px", height:"30px", color:"white" } : {backgroundColor:"white", width:"30px", height:"30px", color:"black"}}> {num} </button>})}


      {/* boton para el Next */}
      <button onClick={() => {numPag(currentPage +1); setCurrentPage(currentPage +1)}} disabled={currentPage===pages} style={currentPage===pages ? {backgroundColor:"#ffffff", color:"gray", cursor:"not-allowed"}: {color:"black"}}> Next </button>
